...
if [ "${is64bit}" != '64' ]; then
    Red_Error "该面板不支持32位操作系统,请使用64位系统安装"
fi

Centos6Check=$(cat /etc/redhat-release | grep ' 6.' | grep -iE 'centos|Red Hat')
if [ "${Centos6Check}" ]; then
    echo "Centos6不支持安装$panel_name，请更换Centos7/8安装$panel_name"
    exit 1
fi

UbuntuCheck=$(cat /etc/issue | grep Ubuntu | awk '{print $2}' | cut -f 1 -d '.')
if [ "${UbuntuCheck}" -lt "16" ]; then
    echo "Ubuntu ${UbuntuCheck}不支持安装$panel_name，建议更换Ubuntu18/20安装$panel_name"
    exit 1
fi
...
Install_Check() {
    ...
    echo -e "----------------------------------------------------"
	echo -e "检查已有其他Web/mysql环境，安装TY可能影响现有站点及数据"
	echo -e "----------------------------------------------------"
	read -p "已知风险/输入yes强制安装: " yes;
	if [ "$yes" != "yes" ];then
		echo -e "------------"
		echo "取消安装"
		exit;
...
Install_Python_Lib() {
    ...
    if [ "${os_version}" != "" ]; then
        ...
        if [ $tmp_size -lt 703460 ]; then
            ...
            if [ ! -f $pyenv_path/pyenv/bin/python ]; then
                rm -f $pyenv_file
                Red_Error "ERROR: 下载宝塔运行环境失败，请尝试重新安装！" 
            fi
            $pyenv_path/pyenv/bin/python3.7 -V
            if [ $? -eq 0 ]; then
                rm -f $pyenv_file
...
Install_Bt() {
    ...
    wget -O panel.zip ${downurl}/panel.zip -T 10
    wget -O /etc/init.d/bt ${downurl}/bt.init -T 10
    wget -O /www/server/panel/install/public.sh ${downurl}/public.sh -T 10
    ...
    if [ ! -f ${setup_path}/server/panel/tools.py ] || [ ! -f ${setup_path}/server/panel/BT-Panel ]; then
        ls -lh panel.zip
        Red_Error "ERROR: 下载宝塔失败，请尝试重新安装！"
    fi
    ...
    wget -O /etc/init.d/bt ${downurl}/bt.init -T 10
    wget -O /www/server/panel/init.sh ${downurl}/bt.init -T 10
    wget -O /www/server/panel/data/softList.conf ${downurl}/softList.conf
}
Other_Openssl() {
    ...
    if [ "$openssl_version" = '1.0.1' ] || [ "$openssl_version" = '1.0.0' ]; then
        ...
        if [ ! -f "/usr/local/openssl/lib/libssl.so" ]; then
            ...
            if [ $tmp_size -lt 703460 ]; then
                ...
                Red_Error "ERROR: 下载 openssl-1.0.2 资源文件失败."
            fi
...
Insatll_Libressl() {
    ...
    if [ "$openssl_version" = '1.0.1' ] || [ "$openssl_version" = '1.0.0' ]; then
        ...
        if [ $tmp_size -lt 703460 ]; then
            rm -f $openssl_src_file
            Red_Error "ERROR: 下载 libressl-$opensslVersion 资源文件失败."
        fi
        tar -zxf $openssl_src_file
        rm -f $openssl_src_file
...
Centos6_Openssl() {
    ...
    if [ $tmp_size -lt 102400 ]; then
        ...
        Red_Error "ERROR: 下载 python env 失败."
    fi
...
while [ "$go" != 'y' ] && [ "$go" != 'n' ]; do
    read -p "确定要安装宝塔面板到 $setup_path 目录下吗?(y/n): " go
done
if [ "$go" == 'n' ]; then
    exit
fi
...
Install_Main
echo -e "=================================================================="
echo -e "\033[32m宝塔面板安装完成!\033[0m"
echo -e "=================================================================="
if [ "$SET_SSL" == true ]; then
    echo "面板地址: https://${getIpAddress}:${panelPort}$auth_path"
else
    echo "面板地址: http://${getIpAddress}:${panelPort}$auth_path"
...
echo -e "用时:\033[32m $outTime \033[0m分!"
